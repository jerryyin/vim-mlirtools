#!/usr/bin/env python3
# Mirrors: https://github.com/llvm/llvm-project/blob/main/mlir/utils/generate-test-checks.py
...
SSA_RESULTS_STR = r'\s*(%' + SSA_RE_STR + \
    r')(\s*,\s*(%' + SSA_RE_STR + r'))*\s*='
...
class VariableNamer:
    def __init__(self, variable_names):
        ...
        self.variable_names = [name.upper()
    ...
    def generate_name(self, source_variable_name):
        ...
        variable_name = self.variable_names.pop(
            0) if len(self.variable_names) > 0 else ''
        if variable_name == '':
...
class AttributeNamer:
    ...
    def __init__(self, attribute_names):
        self.name_counter = 0
        self.attribute_names = [name.upper()
                                for name in attribute_names.split(',')]
        self.map = {}
    ...
    def generate_name(self, source_attribute_name):
        ...
        attribute_name = self.attribute_names.pop(
            0) if len(self.attribute_names) > 0 else ''
        if attribute_name == '':
